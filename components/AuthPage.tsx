...
export const AuthPage: React.FC<AuthPageProps> = ({ state, actions, onAuthSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let success = false;
    if (isLoginView) {
      success = await actions.login(email, password);
    } else {
      success = await actions.register(username, email, password);
    }
    if (success) {
      onAuthSuccess();
    }
